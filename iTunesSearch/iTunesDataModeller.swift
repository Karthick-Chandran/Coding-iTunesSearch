...
//        https://itunes.apple.com/search?term=jack+johnson
        ...
        let url = URL(string: urlWithSearchTerm)!
    ...
    func groupSearchResultByKind() {
        for eachResult in iTunesArray {
            if eachResult.kind != nil {
                if (sections[eachResult.kind!] == nil) {
                    sectionName.append(eachResult.kind!)
                    var dataArr:[iTunesDisplayData] = [iTunesDisplayData]()
                    dataArr.append(eachResult)
                    sections[eachResult.kind!] = dataArr
                }
                else
                {
                    var dataArr = sections[eachResult.kind!]!
                    dataArr.append(eachResult)
                    sections[eachResult.kind!] = dataArr
